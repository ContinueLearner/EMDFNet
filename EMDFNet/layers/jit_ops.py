...
class FastCOCOEvalOp(JitOp):
    ...
    def absolute_name(self):
        return f'EMDFNet.layers.{self.name}'

    def sources(self):
        sources = glob.glob(os.path.join("EMDFNet", "layers", "cocoeval", "*.cpp"))
        if not sources:  # source will be empty list if the so file is removed after install
            # use abosolute path to compile
            import EMDFNet
            code_path = os.path.join(EMDFNet.__path__[0], "layers", "cocoeval", "*.cpp")
            sources = glob.glob(code_path)
        return sources

    def include_dirs(self):
        return [os.path.join("EMDFNet", "layers", "cocoeval")]
